...
async function safeQuery(sql, params = [], label = 'data_trust.query') {
  try {
    const { rows } = await queryWithTimeout(sql, params, { timeoutMs: 10_000, slowQueryMs: 1_000, maxRetries: 0, label });
    return { ok: true, rows };
  } catch (error) {
...
async function buildSummary() {
  ...
  const liveQuotes = await safeQuery(`WITH active AS (SELECT symbol FROM ticker_universe WHERE is_active = true)
        ...
        FROM active a LEFT JOIN market_quotes mq ON mq.symbol = a.symbol`, [], 'data_trust.summary.live_quotes');
  const intraday = await safeQuery(`SELECT COUNT(DISTINCT i.symbol) FILTER (WHERE i.timestamp >= NOW() - INTERVAL '10 minutes')::int AS symbols_10m,
               COUNT(DISTINCT i.symbol) FILTER (WHERE i.timestamp >= NOW() - INTERVAL '24 hours')::int AS symbols_24h,
               MAX(i.timestamp) AS latest
        FROM intraday_1m i JOIN ticker_universe tu ON tu.symbol = i.symbol AND tu.is_active = true`, [], 'data_trust.summary.intraday');
  const daily = await safeQuery(`WITH active AS (SELECT symbol FROM ticker_universe WHERE is_active = true)
        SELECT (SELECT COUNT(*) FROM active)::int AS active_universe,
        ...
        FROM daily_ohlc d JOIN active a ON a.symbol = d.symbol`, [], 'data_trust.summary.daily');
  const news = await safeQuery(`SELECT COUNT(*) FILTER (WHERE published_at >= NOW() - INTERVAL '24 hours')::int AS last_24h,
               MAX(published_at) AS latest
        FROM news_articles`, [], 'data_trust.summary.news');
  const earnings = await safeQuery(`SELECT COUNT(*)::int AS total,
               COUNT(*) FILTER (WHERE eps_estimate IS NOT NULL)::int AS with_est
        FROM earnings_events
        WHERE report_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '14 days'`, [], 'data_trust.summary.earnings');
  const catalysts = await safeQuery(`SELECT COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '1 hour')::int AS last_1h,
               MAX(created_at) AS latest
        FROM catalyst_signals`, [], 'data_trust.summary.catalysts');
...
async function buildSymbol(symbol) {
  const result = await safeQuery(`SELECT tu.symbol, mq.price AS quote_price, mq.updated_at AS quote_updated,
      ...
      WHERE tu.symbol = $1 AND tu.is_active = true`, [symbol], 'data_trust.symbol');
...
router.get('/summary', async (_req, res) => {
  try {
    ...
    return res.status(200).json({ as_of: new Date().toISOString(), health: 'DEGRADED', error: 'data_trust_summary_failed' });
  }
});
...
router.get('/sla', async (_req, res) => {
  try {
    ...
    return res.status(200).json({ as_of: new Date().toISOString(), health: 'DEGRADED', error: 'data_trust_sla_failed' });
  }
});
...
router.get('/symbol/:symbol', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ as_of: new Date().toISOString(), symbol, health: 'DEGRADED', error: 'data_trust_symbol_failed' });
  }
});
