...
function requestJson(url) {
  return new Promise((resolve) => {
    ...
    req.on('error', (err) => {
      resolve({ status: 0, text: String(err), json: null });
    });
  });
}
...
async function main() {
  ...
  console.log(JSON.stringify({ status: out.status, count: out.count, high_volatility_percent: out.high_volatility_percent, pass: out.pass }));
}
