...
export default function SectorHeatmap() {
  ...
  const [narrativesBySector, setNarrativesBySector] = useState({});
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const [heatmapRows, health] = await Promise.all([
          apiJSON('/api/sector/heatmap'),
          apiJSON('/api/system/data-health').catch(() => null),
        ]);

        const sectors = Array.isArray(heatmapRows) ? heatmapRows : [];
        if (!cancelled) {
          setRows(sectors);
          const metricsRows = Number(health?.metrics_rows || 0);
          setPipelineWarmup(metricsRows === 0);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    async function loadNarratives() {
      try {
        ...
        const mapped = items.reduce((acc, row) => {
          const key = String(row?.sector || '').trim();
          if (!key) return acc;
          acc[key] = String(row?.narrative || '');
          return acc;
        }, {});
        if (!cancelled) setNarrativesBySector(mapped);
      } catch {
        if (!cancelled) setNarrativesBySector({});
      }
    }
    ...
    <PageContainer className="space-y-3">
      ...
      <Card>
        ...
        {pipelineWarmup ? (
          ...
          <SectorMarketHeatmap sectors={rows} narrativesBySector={narrativesBySector} />
        )}
      </Card>
      ...
      <Card>
        ...
        {marketData.length > 0 ? (
          ...
          <div className="empty-state">Market data initializing</div>
        )}
      </Card>

      <Card>
        {loading ? (
          <LoadingSpinner message="Market data initializing" />
        ) : rows.length === 0 ? (
          <div className="muted">Market data initializing</div>
        ) : (
              ...
              {rows?.map((row) => {
                return (
                  <tr key={row?.sector || 'unknown'}>
                    ...
                    <td style={{ textAlign: 'right' }}>{fmt(row?.change, 2)}%</td>
